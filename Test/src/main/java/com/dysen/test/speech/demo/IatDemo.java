package com.dysen.test.speech.demo;
...
import com.dysen.test.speech.setting.IatSettings;
import com.dysen.test.speech.util.ApkInstaller;
import com.dysen.test.speech.util.FucUtil;
import com.dysen.test.speech.util.JsonParser;
...
public class IatDemo extends BaseActivity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_iatdemo);
...
//					showTip(getString(R.string.text_begin));
		...
		case R.id.iat_recognize_stream:
			...
			if (ret != ErrorCode.SUCCESS) {
				...
				byte[] audioData = FucUtil.readAudioFile(IatDemo.this, "test3.wav");
	...
	private RecognizerListener mRecognizerListener = new RecognizerListener() {
		...
		public void onResult(RecognizerResult results, boolean isLast) {
			...
			if (isLast) {
				// TODO 最后的结果
				Log.d(TAG, "00000000000000000000000000000000000000000000000000");
			}
		}
		...
		public void onEvent(int eventType, int arg1, int arg2, Bundle obj) {
			...
			//	}
				if (SpeechEvent.EVENT_IST_AUDIO_FILE == eventType) {
					//String sid = obj.getString(SpeechEvent.KEY_EVENT_SESSION_ID);
					//Log.d(TAG, "session id =" + sid);
					showTip("qqqqqqqqqqqqqqqqqqqqqqq");
				}
		}
	};

	private void printResult(RecognizerResult results) {//识别结果
		String text = JsonParser.parseIatResult(results.getResultString());
		String sn = null;
		LogUtils.i("join ---------------------------"+results.toString());
		// 读取json结果中的sn字段
		try {
	...
	private RecognizerDialogListener mRecognizerDialogListener = new RecognizerDialogListener() {
		...
		public void onResult(RecognizerResult results, boolean isLast) {//听写完成
			printResult(results);
		}
