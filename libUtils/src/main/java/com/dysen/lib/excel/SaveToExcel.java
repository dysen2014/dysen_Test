...
public class SaveToExcel {
		...
		for (int rowIndex = 0; rowIndex < readonlySheet.getRows(); rowIndex++) {
			for (int colIndex = 0; colIndex < readonlySheet.getColumns(); colIndex++) {
				...
				if (cell.getType() == CellType.DATE || cell.getType() == CellType.DATE_FORMULA) {
					writableSheet.addCell(new jxl.write.DateTime(colIndex, rowIndex, ((DateCell) cell).getDate(), new WritableCellFormat(cell.getCellFormat())));
				}
				else if (cell.getType() == CellType.NUMBER || cell.getType() == CellType.NUMBER_FORMULA) {
					writableSheet.addCell(new jxl.write.Number(colIndex, rowIndex, ((jxl.NumberCell) cell).getValue(), new WritableCellFormat(cell.getCellFormat())));
				}
					...
					writableSheet.addCell(new Label(colIndex, rowIndex, cell.getContents(), new WritableCellFormat(cell.getCellFormat())));
				}
				else {
	...
	public static void writeExcelUseFormat(String outputFilePath, String outputFileSheetName) throws Exception {
		...
		Label labelC = new Label(0, 1, "This is a Label cell", wcfF);
	...
	public static void setCellValueDirectly(WritableSheet sheet, Cell cell, Object newValue, CellType type) throws Exception {
		if (type == CellType.DATE || type == CellType.DATE_FORMULA) {
			sheet.addCell(new jxl.write.DateTime(cell.getColumn(), cell.getRow(), (Date) newValue, new WritableCellFormat(cell.getCellFormat())));
		}
		else if (type == CellType.NUMBER || type == CellType.NUMBER_FORMULA) {
			sheet.addCell(new jxl.write.Number(cell.getColumn(), cell.getRow(), ((Double) newValue).doubleValue(), new WritableCellFormat(cell.getCellFormat())));
		}
		else if (type == CellType.LABEL || type == CellType.STRING_FORMULA) {
			sheet.addCell(new Label(cell.getColumn(), cell.getRow(), (String) newValue, new WritableCellFormat(cell.getCellFormat())));
		}
		else {
